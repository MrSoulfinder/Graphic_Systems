...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		positionX = (((float)rand()-(float)rand())/RAND_MAX) * m_particleDeviationX * 4;
...
void ParticleSystemClass::UpdateParticles(float frameTime)
{
	...
	for(i=0; i<m_currentParticleCount; i++)
	{
		...
		if (m_particleList[i].positionY > -10) {
			m_particleList[i].red = { 1.0 };
			m_particleList[i].blue = { 0.0 };
			m_particleList[i].green = { 0.0 };
		}
		if (m_particleList[i].positionY > 2) {
			m_particleList[i].red = { 1.0 };
			m_particleList[i].blue = { 0.0 };
			m_particleList[i].green = { 0.5 };
		}
		if (m_particleList[i].positionY > 5) {
			m_particleList[i].red = { 0.0 };
			m_particleList[i].blue = { 0.0 };
			m_particleList[i].green = { 0.0 };
		}
	}
