...
const float defG = 0.0f; // Гравитационная постоянная умноженная на массу "курсора"
const float defResistance = 0.995f; // Сопротивление среде
//////////
...
#pragma omp for nowait
		for(int i = 0; i < size; i += 4)
		{
			...
			if(distSquare.m256_f32[0] < 1000)
			{
				theForce.m256_f32[0] = 0;
				theForce.m256_f32[1] = 0;
				/*if (distSquare.m256_f32[0] > 700) {
					theForce.m256_f32[0] = -0.1;
					theForce.m256_f32[1] = -0.1;
				}*/
			}

			if(distSquare.m256_f32[2] < 1000)
			{
				theForce.m256_f32[2] = 0;
				theForce.m256_f32[3] = 0;
				/*if (distSquare.m256_f32[2] < 700) {
					theForce.m256_f32[2] = -0.1;
					theForce.m256_f32[3] = -0.1;
				}*/
			}
			if(distSquare.m256_f32[4] < 1000)
			{
				theForce.m256_f32[4] = 0;
				theForce.m256_f32[5] = 0;
				/*if (distSquare.m256_f32[4] < 700) {
					theForce.m256_f32[4] = -0.1;
					theForce.m256_f32[5] = -0.1;
				}*/
			}

			if(distSquare.m256_f32[6] < 1000)
			{
				theForce.m256_f32[6] = 0;
				theForce.m256_f32[7] = 0;
				/*if (distSquare.m256_f32[6] < 700) {
					theForce.m256_f32[6] = -0.1;
					theForce.m256_f32[7] = -0.1;
				}*/
			}
			...
			/*pVertexBuffer[i].life_time -= 1;
			if (pVertexBuffer[i].life_time <= 0) {
				pVertexBuffer[i].x = 0;
				pVertexBuffer[i].y = 0;
			}
			else if (pVertexBuffer[i+1].life_time <= 0) {
				pVertexBuffer[i + 1].x = 0;
				pVertexBuffer[i + 1].y = 0;
			}
			else if (pVertexBuffer[i+2].life_time <= 0) {
				pVertexBuffer[i + 2].x = 0;
				pVertexBuffer[i + 2].y = 0;
			}
			else if (pVertexBuffer[i+3].life_time <= 0) {
				pVertexBuffer[i + 3].x = 0;
				pVertexBuffer[i + 3].y = 0;
			}*/
...
void initParticles()
{
	...
	for(int i = 0; i < partCount; ++i)
	{
		/*int chapter = rand() % 7;
		if (chapter == 0){
			particlesCoord[i].x = (rand() % (Width/20))+Width/4;
			particlesCoord[i].y = (rand() % (Height/2))+Height/4;
		}
		else if (chapter == 1) {
			particlesCoord[i].x = (rand() % (Width / 8)) + Width / 4 + Width / 20;
			particlesCoord[i].y = (rand() % (Height / 10)) + Height / 2 - Height/20 ;
		}
		else if (chapter == 2) {
			particlesCoord[i].x = (rand() % (Width / 20)) + Width / 4 + Width/8+Width/20;
			particlesCoord[i].y = (rand() % (Height / 2)) + Height / 4;
		}
		else if (chapter == 3) {
			particlesCoord[i].x = (rand() % (Width / 20)) + Width / 4 + Width / 8 + Width / 20+ Width/20+Width/20;
			particlesCoord[i].y = (rand() % (Height / 10)) + Height / 4;
		}
		else if (chapter == 4) {
			particlesCoord[i].x = (rand() % (Width / 20)) + Width / 4 + Width / 8 + Width / 20 + Width / 20 + Width / 20;
			particlesCoord[i].y = (rand() % (Height / 3)) + Height / 4+Height/6;
		}
		else if (chapter == 5) {
			particlesCoord[i].x = (rand() % (Width / 20)) + Width / 4 + Width / 8 + Width / 20 + Width / 20 + Width / 20 + Width/10;
			particlesCoord[i].y = (rand() % (Height / 3)) + Height / 4;
		}
		else if (chapter == 6) {
			particlesCoord[i].x = (rand() % (Width / 20)) + Width / 4 + Width / 8 + Width / 20 + Width / 20 + Width / 20 + Width / 10;
			particlesCoord[i].y = (rand() % (Height / 10)) + Height / 2 + Height/7;
		}*/
		...
		//particlesCoord[i].life_time = rand() % 500;
		particlesVel[i].vx = rand() % 2;
		particlesVel[i].vy = rand() % 2;
		/*particlesVel[i].vx = 10;
		particlesVel[i].vy = 20;*/
...
void initD3D()
{
	...
	//D3DXCOLOR color = D3DXCOLOR(red, green, blue, 0);
...
void initVertexData()
{
	...
	for(size_t i = 0; i < count; ++i)
	{
		...
		vertexData[i].u = 0;
		vertexData[i].v = 0;
		vertexData[i].life_time = rand() % 10;
	}
